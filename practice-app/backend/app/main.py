...
from .questions import get_question, list_sections
...
@app.get("/api/health")
def health() -> dict:
    """Liveness + Firestore reachability."""
    try:
        # Cheap probe: list 1 doc from `questions`.
...
def exams() -> dict:
    return {
        "exams": [
            {"id": "pca", "name": "Professional Cloud Architect", "sections": list_sections("pca")},
            {"id": "devops", "name": "Professional Cloud DevOps Engineer", "sections": list_sections("devops")},
        ]
    }
...
def start(req: StartSessionRequest) -> SessionStartResponse:
    try:
        result = sessions_svc.start_session(req.exam, req.num_questions, req.sections)
    ...
    return SessionStartResponse(
        ...
        total=result["total"],
        first_question=QuestionForClient(**q.model_dump(exclude={"explanation"})),
    )
...
def answer(sid: str, req: AnswerRequest) -> AnswerResponse:
    ...
    if result["next_qid"]:
        ...
        if nq:
            next_q = QuestionForClient(**nq.model_dump(exclude={"explanation"}))
